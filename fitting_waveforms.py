...
t_plot = nr_waveform["t"][start_ind:] - nr_waveform["t"][start_ind]
abs_plot = np.abs(nr_waveform["hlm"][(2, 2)])[start_ind:]
...
def func(t, ecc, f_min):
    ...
    #     plt.plot(t, np.abs(nr_waveform["hlm"][(2,2)]), label = 'NR')
...
def do_fit():
    xtol, gtol, ftol = 1e-10, 1e-10, 1e-10
    initial_guess = np.array(
        [nr_results["eccentricity"], nr_results["fref_out"] / lal.MTSUN_SI / Mtot]
    )

    popt, pcov, infodict, mesg, ier = scipy.optimize.curve_fit(
        func,
        nr_waveform["t"],
        np.abs(nr_waveform["hlm"][(2, 2)]),
        p0=initial_guess,
        bounds=(0.94 * initial_guess, 1.06 * initial_guess),
        method="trf",
        full_output=True,
        gtol=gtol,
        xtol=xtol,
    ...
    plt.plot(t_plot, func(t_plot, *popt), label="Fitted EOB")
    plt.plot(t_plot, abs_plot, label="NR")
    plt.legend()
